...
void dot_output::gen()
{
    ...
    out << "digraph finite_state_machine {\n"
        << "label=\"Document: " << htmlEscape(sc.sc().name) << "\\l"
        << "Date: " << std::put_time(std::localtime(&currentTime), "%F") << "\\l\""
        << "\tnode [shape = Mrecord];\n"
        << "\tcompound=true;\n"
        << "\tsize=\"8,5;\"\n";
...
void dot_output::gen_states()
{
    ...
    if(! sc.sc().initial.target.empty())
    {
        ...
        out << "_start[label=\"\",shape=\"circle\",style=filled,fixedsize=\"true\",fillcolor=\"black\",width=\"0.2\"]\n";
